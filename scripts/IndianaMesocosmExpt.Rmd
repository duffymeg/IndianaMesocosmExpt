...
propinfovertimeplo<-ggplot(indianadatametsch,aes(time,y=propinfected,group=chaoborusfactor,color=chaoborusfactor,shape=chaoborusfactor)) +
...
propinfovertimeplot <- annotate_figure(propinfovertimeplo, 
                          left = text_grob(expression(atop(Proportion~infected)), just = "center", family = "sans", rot=90)) +
                          theme(plot.margin = margin(0.1, 0.1, 0.1, 0.2, "lines"))
propinfovertimeplot
...
infdensityovertimeplo<-ggplot(indianadatametsch,aes(time,y=infectedlnone,group=chaoborusfactor,color=chaoborusfactor,shape=chaoborusfactor)) +
  stat_summary(fun="mean",geom="line",size=1, show.legend = FALSE) +
  stat_summary(fun.data = "mean_se",show.legend=FALSE) + 
...
infdensityovertimeplot <- annotate_figure(infdensityovertimeplo, 
                          left = text_grob(expression(atop(Infected~prey~density,"(*LN prey per L + 1)")), just = "center", family = "sans", rot=90)) +
                          theme(plot.margin = margin(0.1, 0.1, 0.1, 0.2, "lines"))
...
meaninfprevplo <- ggplot(infprevsummaryanalysis,aes(x=chaoborusfactor,y=meanpropinf,color=chaoborusfactor)) +
...
meaninfprevplot <- annotate_figure(meaninfprevplo, 
                          left = text_grob(expression(atop(Average,proportion~infected)), just = "center", family = "sans", rot=90)) +
                          theme(plot.margin = margin(0.1, 0.1, 0.1, 0.2, "lines"))
meaninfprevplot
...
meaninfhostplo <- ggplot(infdenssummaryanalysis,aes(x=chaoborusfactor,y=meaninfdens,color=chaoborusfactor)) +
  geom_boxplot(show.legend=FALSE) +
  geom_jitter(shape=17, position=position_jitter(width=0.2, height=0), alpha = 0.5, color = '#787878', size = 2) +
  labs(x="Predation treatment (predators per L)",y="Average infected prey density \n(LN prey per L + 1)") +
  scale_color_manual(values=c("#6baed6", "#3182bd", "#08519c","black")) +
  theme_bw() +
  geom_text(data=infdenssummaryanalysis,x=2.3, y=0.18,label ="Predation: p < 0.0001", color="black",size = 2.5) +
  ylim(0.0,1.6) +   
  theme(axis.text.x = element_blank(),
  axis.title.x = element_blank(), 
  axis.title.y = element_blank(), 
...
meaninfhostplot <- annotate_figure(meaninfhostplo, 
                          left = text_grob(expression(atop(Ave.~infected~prey~density,"(LN prey per L + 1)")), just = "center", family = "sans", rot=90)) +
                          theme(plot.margin = margin(0.1, 0.1, 0.1, 0.2, "lines"))
meaninfhostplot
...
popdensitytimeseriesmetsch <- ggplot(indianadatametsch, aes(x = time, y = (logone), color = chaoborusfactor, group = chaoborusfactor, shape = chaoborusfactor)) + 
  ...
  axis.title.x = element_blank(), 
  axis.title.y = element_blank(), 
  plot.margin = margin(0.5, 0.5, 0.5, 1.0, "lines"), 
...
meandensityplo <- ggplot(densitysummaryanalysis,aes(x=chaoborusfactor,y=meandens, color=chaoborusfactor, fill = metsch)) +
  ...
  ylim(0.0,4.6) + 
  xlab("Predator Density per L")+
  theme(panel.grid=element_blank(), 
        axis.title.y = element_blank(), 
        legend.position = c(0.2, 0.38), 
        legend.key.size = unit(0.5, 'cm'),
        legend.title = element_blank(),
        legend.text = element_text(size=7), 
        plot.margin = margin(0.5, 0.5, 0.5, 1.0, "lines"))

meandensityplot <- annotate_figure(meandensityplo, 
                          left = text_grob(expression(atop(Ave.~prey~density,"(LN prey per L + 1)")), just = "center", family = "sans", rot=90)) +
                          theme(plot.margin = margin(0.1, 0.1, 0.1, 0.2, "lines"))
meandensityplot
...
metschresevolplotgroupe <- ggplot(evoldata2,aes(round,y=infectionmean_scaled,group=para,color=para,shape=para)) + 
  stat_summary(fun="mean",geom="line",size=1,show.legend=FALSE) +
  stat_summary(fun.data = "mean_se",show.legend=FALSE) +
...
metschresevolplotgrouped <- annotate_figure(metschresevolplotgroupe, 
                          left = text_grob(expression("Ave. infection rate, " *beta* ", ("*"*10"^"-7"*")"), just = "center", family = "sans", rot=90)) +
                          theme(plot.margin = margin(0.1, 0.1, 0.1, 0.2, "lines"))
...
totaldensityfigure <- ggarrange(plotlist = list(popdensitytimeseriesmetsch,popdensitytimeseriesnometsch),
                    nrow = 2, ncol=1,
                    heights = c(1/2, 1/2), 
                    align = "v"
                    ) 

totaldensityfigurelabeled <- annotate_figure(totaldensityfigure, 
                          left = text_grob(expression(atop(Prey~density,LN~prey~per~L~+~1)), just = "center", family = "sans", rot=90)) +
                          theme(plot.margin = margin(0.1, 0.1, 0.1, 0.2, "lines"))

metschresevolplot <- ggarrange(plotlist = list(metschresevolplotmetsch,metschresevolplotnometsch),
                    nrow = 2, ncol=1,
                    heights = c(1/2, 1/2), 
                    align = "v"
                    ) 
metschresevolplotlabeled <- annotate_figure(metschresevolplot, 
                          left = text_grob(expression(atop("Ave. infection rate,"~beta~"("*"*10"^"-7"*")")), just = "center", family = "sans", rot=90)) +
                          theme(plot.margin = margin(0.1, 0.1, 0.1, 0.2, "cm"))

F1_RHS <- ggarrange(plotlist = list(propinfovertimeplot, infdensityovertimeplot, totaldensityfigurelabeled, metschresevolplotlabeled),
                    nrow = 4, ncol=1 ,
                    heights = c(1/4, 1/4, 1/4, 1/4)
                    )

F1_LHS <- ggarrange(plotlist = list(meaninfprevplot, meaninfhostplot, meandensityplot, metschresevolplotgrouped),
                    nrow = 4, ncol=1 ,
                    heights = c(1/4, 1/4, 1/4, 1/4)
                    )

figure1 <- ggarrange(
  F1_RHS + theme(plot.margin = unit(c(1, 0.5, 0, 0), "lines")),
  F1_LHS + theme(plot.margin = unit(c(1, 0, 0, 0.5), "lines")),
  ncol = 2, nrow=1,
  widths = c(1, 1)
  )
...
# Stage structure -- maybe put this in a supplement
...
chloroplot <- ggplot(chlorodata,aes(time,y=chloroln,group=chaoborus,color=chaoborus,shape=chaoborus)) +
  stat_summary(fun="mean",geom="line",size=1,show.legend=FALSE) +
...
chlaov2b <- aov(meanlnchla~chaoborus*metsch,data=chlasummarysecondhalf)
summary(chlaov2b)  

overdisp_fun(chlaov2b)
# super overdispersed
```
...
secondhalfchlsummaryplot <- ggplot(chlaov2b,aes(x=chaoborus,y=meanlnchla, color=chaoborus, fill = metsch)) +
  geom_boxplot(lwd=0.75) +
  geom_point(pch=17, position=position_jitterdodge(jitter.width = 0.2), alpha = 0.5, size = 2, color = 'black',show.legend=FALSE) +
    geom_text(data=chlaov2b,x=1.5, y=0.75,label ="Predation: p < 0.0001\nParasitism: p = 0.50\nPred*Para: p = 0.16", color="black",size = 2.5) +
  labs(y="Average LN chlorophyll \nweeks 6-9 (ug/L)") +
  labs(color = "Predator density \n(per L)", fill = "Parasite treatment") +
  scale_color_manual(values=c("#6baed6", "#3182bd", "#08519c","black"), guide="none") +
  scale_fill_manual(values=c('white','gray'))  +
  theme_bw() +   
...
eggsummarysamplesizes

eggsummarysamplesizes_25 <- eggs %>%
  filter(time == 2 | time == 3 | time == 4 | time == 5) %>%
  group_by(pred, metsch, tank) %>%
  summarise(n = n()) 

eggsummarysamplesizes_25

eggsummarysamplesizes_69 <- eggs %>%
  filter(time == 6 | time == 7 | time == 8 | time == 9) %>%
  group_by(pred, metsch, tank) %>%
  summarise(n = n()) 

eggsummarysamplesizes_69
...
eggaov25 <- aov(meaneggs~pred*metsch,data=eggsummary25)
summary(eggaov25)  
...
eggaov69 <- aov(meaneggs~pred*metsch,data=eggsummary69)
summary(eggaov69)  
```
...
eggsummaryplot <- ggplot(eggaov1,aes(x=pred,y=meaneggs, color=pred, fill = metsch)) +
  geom_boxplot(lwd=0.75) +
  geom_point(pch=17, position=position_jitterdodge(jitter.width = 0.2), alpha = 0.5, size = 2, color = 'black',show.legend=FALSE) +
      geom_text(data=eggaov1,x=2.3, y=2.62,label ="Predation: p < 0.0001\nParasitism: p = 0.16\nPred*Para: p = 0.50", color="black",size = 2.5) +
...
```{r, egg plots split by time}
eggsummaryplot25 <- ggplot(eggaov25,aes(x=pred,y=meaneggs, color=pred, fill = metsch)) +
  geom_boxplot(lwd=0.75) +
  geom_point(pch=17, position=position_jitterdodge(jitter.width = 0.2), alpha = 0.5, size = 2, color = 'black',show.legend=FALSE) +
      geom_text(data=eggaov25,x=1.5, y=4.45,label ="Predation: p < 0.0001\nParasitism: p = 0.071\nPred*Para: p = 0.56", color="black",size = 2.5) +
...
eggsummaryplot69 <- ggplot(eggaov69,aes(x=pred,y=meaneggs, color=pred, fill = metsch)) +
  geom_boxplot(lwd=0.75) +
  geom_point(pch=17, position=position_jitterdodge(jitter.width = 0.2), alpha = 0.5, size = 2, color = 'black',show.legend=FALSE) +
      geom_text(data=eggaov69,x=1.5, y=4.45,label ="Predation: p = 0.086\nParasitism: p = 0.91\nPred*Para: p = 0.49", color="black",size = 2.5) +
  labs(x="Predator density (per L)", y="Average egg ratio weeks 6-9 \n(embryos per uninfected adult)") +
  labs(color = "Predator density \n(per L)", fill = "Parasite treatment") +
  scale_color_manual(values=c("#6baed6", "#3182bd", "#08519c","black"), guide="none") +
  ylim(0,5) +
  scale_fill_manual(values=c('white','gray')) +
  ...
  theme(legend.key.size = unit(0.5, 'cm'), legend.title = element_blank(), #change legend title font size
        legend.text = element_text(size=7))  
...
```{r, making figure 3}
figure3 <- plot_grid(chloroplot, firsthalfchlsummaryplot, secondhalfchlsummaryplot, eggplot, eggsummaryplot25, eggsummaryplot69, labels = c('a', 'b', 'c', 'd', 'e', 'f'), ncol = 3, rel_heights = c(0.9, 1))
figure3
...
## Combining to make old figure 4
```{r, making old figure 4 which is now in the supplement}
evolutionplot <- plot_grid(traitplot, predresevolplot, predresevolplottogether, labels = "auto", ncol = 1)
evolutionplot
