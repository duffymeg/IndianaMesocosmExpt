...
metschresevolplot <- ggplot(evoldata2,aes(round,y=infectionmean_scaled,group=pred,color=pred,shape=pred)) + 
  ...
  ylim(0.5, 1.6) +
  labs(x="Time (weeks)") +
  labs(y=bquote('Infection suscept.'~(x10^7))) + 
...
metschresevolplotgrouped <- ggplot(evoldata2,aes(round,y=infectionmean_scaled,group=para,color=para,shape=para)) + 
  ...
  theme(panel.grid = element_blank()) +
  labs(x="Time (weeks)") +
  labs(y=bquote('Ave. infection suscept. '~(x10^7))) + 
  scale_x_continuous(breaks=c(0,2,6,9)) +  
  ylim(0.5, 1.6) +
  theme(axis.title.y = element_text(margin = margin(t = 0, r = 10, b = 0, l = 0))) +
  theme(axis.title.x=element_text(margin=margin(t=10,r=0,b=0,l=0))) +
...
figure1 <- plot_grid(propinfovertimeplot, meaninfprevplot, infdensityovertimeplot, meaninfhostplot, metschresevolplot, metschresevolplotgrouped, labels = "auto", ncol = 2, rel_heights = c(0.9, 1, 1))
figure1

ggsave(here("figures", "figure1.jpg"), figure1, units = "in", width = 7, height = 8, dpi = 300)
```
...
meandensityplot <- ggplot(densitysummaryanalysis,aes(x=chaoborusfactor,y=meandens, color=chaoborusfactor, fill = metsch)) +
  ...
  labs(x="Predator density (per L)",y="Average prey density \n(LN prey per L + 1)") +
...
#  geom_text(data=densitysummaryanalysis,x=4,y=4.6,label = "b              b", color = "black",size = 3) +
  geom_text(data=densitysummaryanalysis,x=2.5, y=0.5,label ="Predation: p < 0.0001\nParasitism: p = 0.12\nPred*Para: p = 0.49", color="black",size = 2.5) +
  ylim(0.0,4.6) + 
  theme(panel.grid=element_blank()) +
      theme(legend.position = c(0.19, 0.38)) +
...
```{r, uninfected population size over time}
indianadata$uninfdensity <- indianadata$totaldensity - indianadata$densityinfected
indianadata$lnuninfdensityplusone <- log(indianadata$uninfdensity + 1)

uninfdensityplot <- ggplot(indianadata, aes(x = time, y = (lnuninfdensityplusone), color = chaoborusfactor, group = chaoborusfactor, shape = chaoborusfactor)) + 
  stat_summary(fun = "mean", geom="line", aes(group = chaoborusfactor)) + 
  stat_summary(fun.data="mean_se") +
    stat_summary(fun="mean",geom="point",size=4,aes(shape=chaoborusfactor),show.legend=FALSE) +
  facet_grid(rows=vars(metsch)) +
  labs(x="Time (week)",y="Uninfected prey density \n(LN prey per L + 1)") + 
  scale_x_discrete(limits=c("1", "2","3","4","5","6","7","8","9")) + 
  scale_color_manual(values=c("#6baed6", "#3182bd", "#08519c","black")) +
    scale_shape_manual(values=c(15,16,17,18)) +
  labs(color="Pred.", shape = "Pred.") +
...
```
## Average susceptible host density analysis

```{r, average susceptible host density analysis}
# remove time 1 for analysis
indianadataanalysis <- indianadata %>%
  filter(time > 1)

uninfdensitysummary <- indianadataanalysis %>%
  group_by(chaoborusfactor, metsch, tank) %>%
  summarise(meanuninfdens = mean(lnuninfdensityplusone)) 
  
uninfdensitysummaryanalysis <- aov(meanuninfdens~chaoborusfactor*metsch,data=uninfdensitysummary)
summary(uninfdensitysummaryanalysis)  

overdisp_fun(uninfdensitysummaryanalysis)
#not overdispersed

lsmeans(uninfdensitysummaryanalysis,pairwise~chaoborusfactor*metsch,adjust="tukey")

```

## Average uninf host density plot

```{r, average uninf prey density plot}
meanuninfdensityplot <- ggplot(uninfdensitysummaryanalysis,aes(x=chaoborusfactor,y=meanuninfdens, color=chaoborusfactor, fill = metsch)) +
  geom_boxplot(lwd=0.75) +
  geom_point(pch=17, position=position_jitterdodge(jitter.width = 0.2), alpha = 0.5, size = 2, color = 'black',show.legend=FALSE) +
  labs(x="Predator density (per L)",y="Average uninf. prey density \n(LN prey per L + 1)") +
  labs(color = "Predator density \n(per L)", fill = "Parasite treatment") +
  scale_color_manual(values=c("#6baed6", "#3182bd", "#08519c","black"), guide="none") +
  scale_fill_manual(values=c('white','gray')) +
  theme_bw() +   
  geom_text(data=uninfdensitysummaryanalysis,x=2.5, y=0.75,label ="Predation: p < 0.0001\nParasitism: p = 0.024\nPred*Para: p = 0.36", color="black",size = 2.5) +
#  ylim(0.0,4.6) + 
  theme(panel.grid=element_blank()) +
      theme(legend.position = c(0.19, 0.4)) +
  theme(legend.key.size = unit(0.5, 'cm'), legend.title = element_blank(),
        legend.text = element_text(size=7)) 

meanuninfdensityplot
...
figure2 <- plot_grid(figure2parta, meandensityplot, 
                     uninfdensityplot, meanuninfdensityplot, labels = "auto", ncol = 2)
figure2

ggsave(here("figures", "figure2.jpg"), figure2, units = "in", width = 7, height = 5.5, dpi = 300)
```
...
traitplot<-ggplot(traitdata, aes(x=inf.susc,y=pred.susc)) +
  ...
  theme_bw()+theme(panel.grid = element_blank()) + 
  labs(x=bquote('Infection susceptibility'~(x10^7))) +
  labs(y='Predation suscept.') 
...
predresevolplot<-ggplot(evoldata2,aes(round,y=predationmean,group=pred,color=pred,shape=pred)) + 
  ...
  scale_colour_manual(values=c("#6baed6", "#3182bd", "#08519c","black")) +
  theme_bw()+theme(panel.grid = element_blank()) + 
  labs(x="Time (weeks)")+labs(y="Ave. pred. suscept.") +
  theme(axis.title.y = element_text(margin = margin(t = 0, r = 10, b = 0, l = 0))) +
  theme(axis.title.x=element_text(margin=margin(t=10,r=0,b=0,l=0))) +
  scale_x_continuous(breaks=c(0,2,6,9)) +
  facet_grid(rows=vars(para)) +
